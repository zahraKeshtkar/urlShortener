...
func (suite *URLTestSuite) Test_MakeShortURL_Success() {
...
func (suite *URLTestSuite) Test_URLValidate_Success() {
	require := suite.Require()
	testCases := [2]model.Link{{URL: "www.google.com"}, {URL: "https://linuxhint.com/"}}

	for _, link := range testCases {
		require.Equal(true, link.Validate())
	}
}

func (suite *URLTestSuite) Test_ShortURLValidate_Success() {
	require := suite.Require()
	shortURL := model.Link{ShortURL: "ZZZZZZZv"}

	require.Equal(true, shortURL.Validate())
}

func (suite *URLTestSuite) Test_ShortURLToID_Success() {
	require := suite.Require()
...
func (suite *URLTestSuite) Test_URLValidate_Fail() {
	require := suite.Require()
	testCases := [3]model.Link{{ShortURL: ""},
		{ShortURL: "ww.google"},
		{ShortURL: "wwe.google.com"}}

	for _, link := range testCases {
		require.Equal(false, link.Validate())
	}
}

func (suite *URLTestSuite) Test_ShortURLValidate_Fail() {
	require := suite.Require()
...
func (suite *URLTestSuite) Test_MakeShortURL_Fail() {
	require := suite.Require()
...
func (suite *URLTestSuite) Test_ShortURLToID_Fail() {
	require := suite.Require()
